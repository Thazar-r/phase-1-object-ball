function gameObject() {
  ...
  function playerNumbers(teamName) {
    ...
    for (let teamKey in game) {
      if (game[teamKey].teamName === teamName) {
        teamPlayers = Object.keys(game[teamKey].players).map(
          player => game[teamKey].players[player].number
        );
      }
    }
    return teamPlayers;
    ...
    for (let teamKey in game) {
      for (let player in game[teamKey].players) {
        if (game[teamKey].players[player].shoe > largestShoeSize) {
          ...
          playerWithLargestShoes = player;
        }
      }
    }
  
    return game.home.players[playerWithLargestShoes].rebounds;
  }
